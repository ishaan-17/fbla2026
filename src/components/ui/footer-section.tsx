...
export function Footer() {
	return (
		<footer className="relative w-full border-t border-white/10 bg-neutral-900 overflow-hidden px-6 py-12 lg:py-16">
			...
			<div className="grid w-full max-w-6xl mx-auto grid-cols-1 md:grid-cols-5 gap-8">
				...
				{footerLinks.map((section, index) => (
					<AnimatedContainer key={section.label} delay={0.1 + index * 0.1}>
						<div>
							<h3 className="text-xs font-semibold text-white/50 uppercase tracking-wider">{section.label}</h3>
							<ul className="mt-4 space-y-2 text-sm">
								{section.links.map((link) => (
									<li key={link.title}>
										...
										>
											{link.icon && <link.icon className="me-2 size-4 text-white/40" />}
											{link.title}
										</a>
									</li>
								))}
